...
namespace ImageTools
{
    public class PointTransformation
    {
        ...
        public static void AddValue(DirectBitmap bitmap, Channel channels, int value)
        {
            for (int i = 0; i < bitmap.Height * bitmap.Width; i++)
            {
                var color = Color.FromArgb(bitmap.Bits[i]);
                ...
                int G = color.G;
                int B = color.B;
                if ((Channel.Red & channels) == Channel.Red)
                {
                    AddValueToChannel(ref R, ref _realChannelValues[0, i], value);
                }
                if ((Channel.Green & channels) == Channel.Green)
                {
                    AddValueToChannel(ref G, ref _realChannelValues[1, i], value);
                }
                if ((Channel.Blue & channels) == Channel.Blue)
                {
                    AddValueToChannel(ref B, ref _realChannelValues[2, i], value);
                }
                bitmap.Bits[i] = Color.FromArgb(R, G, B).ToArgb();
            }
        }

        public static void MultiplyByValue(DirectBitmap bitmap, Channel channels, double value)
        {
            for (int i = 0; i < bitmap.Height * bitmap.Width; i++)
            {
                var color = Color.FromArgb(bitmap.Bits[i]);
                ...
                if ((Channel.Red & channels) == Channel.Red)
                {
                    MultiplyChannelByValue(ref R, ref _realChannelValues[0, i], value);
                }
                if ((Channel.Green & channels) == Channel.Green)
                {
                    MultiplyChannelByValue(ref G, ref _realChannelValues[1, i], value);
                }
                if ((Channel.Blue & channels) == Channel.Blue)
                {
                    MultiplyChannelByValue(ref B, ref _realChannelValues[2, i], value);
                }
                bitmap.Bits[i] = Color.FromArgb(R, G, B).ToArgb();
            }
        }
        public static void ConvertToGray(DirectBitmap bitmap, GrayConversionMode mode)
        {
            for (int i = 0; i < bitmap.Height * bitmap.Width; i++)
            {
                ...
                _originalChannelValues[0, i] = R;
                _originalChannelValues[1, i] = G;
                _originalChannelValues[2, i] = B;
                bitmap.Bits[i] = Color.FromArgb(R, G, B).ToArgb();
            }
        }
